...
**Mods:** Config lib, Dear ImGui, Anvil Performance Optimization, xLib Fork

Nothing to actually play with here — all four are backend dependency code with no player-visible screens, items, or interactions of their own. They exist purely so other mods in this list can have something to build on:
...
- **xLib Fork** — the skill/ability/effect framework and client-server sync layer that [xSkills Fork](progression.md) is built on. Installed solely as its dependency.
