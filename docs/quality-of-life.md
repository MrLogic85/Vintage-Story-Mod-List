...
**Known overlap with xSkills Fork** ([Progression](progression.md)): both mods default their HUD elements to the top-left corner, so xSkills' effect box (buff icons like "Well Rested") can overlap HUD Clock's panel. Fix: in `ModConfig/XLeveling/gui/xSkillsGilded.json`, set `effectBoxOriginX`/`effectBoxOriginY` (default `8`/`8`) to move the box below HUD Clock's panel — `13`/`122` is the value that lines up cleanly at **2560×1440**. These are raw pixel coordinates, not values relative to your resolution or UI scale, so they likely need retuning at other resolutions/UI scales — nudge `effectBoxOriginY` until the box clears HUD Clock's panel again if it overlaps at yours.
